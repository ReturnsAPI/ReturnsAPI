...
Instance.wrap = function(id)
    ...
    if inst.value == -4 then    -- Invalid instance
...
methods_instance = {
    ...
    exists = function(self)
        ...
        if not ret then Proxy.set(self, -4) end -- Make this wrapper invalid if the instance actually existn't
        return ret
    end,
...
--     for k, v in pairs(__instance_data) do
--         if gm.instance_exists(k) == 0 then
--             __instance_data[k] = nil
--         end
--     end
-- end)
...
--     if __instance_data[id] then
--         __instance_data[args[2].value.id] = __instance_data[id]
