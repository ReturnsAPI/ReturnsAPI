-- this file is an alternative approach to RecalculateStats, using mid-function hooking instead of recreating recalculate_stats
-- it performs significantly better at a glance, and is certainly less bloated.

-- this table would be used internally to store everything related to the current recalculate_stats operation
local params = {
	--skill_primary = {},
	skill_secondary = {},
	skill_utility = {},
...
-- instead of directly exposing the params table, a key-locked table like this one should be exposed
-- this enforces a saner API and protects against breakage (if the params table gets messed up, it becomes everyone's problem and could be fatal)
local api = {
	maxhp_add = function(value)
		params.maxhp_add = params.maxhp_add + value
	end,
	maxhp_mult = function(value)
		params.maxhp_mult = params.maxhp_mult * value
	end,
	-- ... and so on. perhaps the functions could be automatically generated on load from the param table's fields?
}
...
gm.pre_script_hook(gm.constants.recalculate_stats, gather_params)

-- it might not be necessary to mid hook for regen, damage, crit chance, lifesteal, armor, explosive shot, lightning, fire trail
-- because nothing in the script depends on those values, so it should be safe to set them after the script
-- if it's faster/better to have one final hook handle all of those in one go, then this should be changed to reflect that

-- stat dependencies:
-- attack speed and cdr is used by ActorSkills (is_primary skills reduce cooldown with attack speed)
-- pHmax is used to calculate acceleration and friction
-- pVmax and pGravity2 is used in the rusty jetpack calculations

local ptr = gm.get_script_function_address(gm.constants.recalculate_stats)

memory.dynamic_hook_mid("RecalculateStats-knockback_cap", {"rbx"}, {"RValue*"}, 0, ptr:add(0x96d), function(args)
	args[1].value = (args[1].value + params.knockback_cap_add) * params.knockback_cap_mult
end)

memory.dynamic_hook_mid("RecalculateStats-cdr", {"rdi"}, {"RValue*"}, 0, ptr:add(0xa6c), function(args)
...
memory.dynamic_hook_mid("RecalculateStats-maxhp", {"rcx"}, {"RValue*"}, 0, ptr:add(0xd7e), function(args)
...
memory.dynamic_hook_mid("RecalculateStats-hp_regen", {"rax"}, {"RValue*"}, 0, ptr:add(0x22e7), function(args)
	-- no multiplicative regen modifiers in vanilla
	args[1].value = (args[1].value + params.hp_regen_add) * params.hp_regen_mult
end)
memory.dynamic_hook_mid("RecalculateStats-damage", {"rax"}, {"RValue*"}, 0, ptr:add(0x156b), function(args)
	-- runs between vanilla additive and multiplicative modifiers, so both operations can be in the same hook
...
memory.dynamic_hook_mid("RecalculateStats-maxshield", {"rax"}, {"RValue*"}, 0, ptr:add(0x1bdf), function(args)
...
memory.dynamic_hook_mid("RecalculateStats-critical_chance", {"rdx"}, {"RValue*"}, 0, ptr:add(0x28c7), function(args)
...
memory.dynamic_hook_mid("RecalculateStats-lifesteal", {"rax"}, {"RValue*"}, 0, ptr:add(0x2965), function(args)
	-- no multiplicative lifesteal modifiers in vanilla
	args[1].value = (args[1].value + params.lifesteal_add) * params.lifesteal_mult
end)

memory.dynamic_hook_mid("RecalculateStats-attack_speed", {"rdx"}, {"RValue*"}, 0, ptr:add(0x3034), function(args)
...
memory.dynamic_hook_mid("RecalculateStats-pHmax", {"rax"}, {"RValue*"}, 0, ptr:add(0x3c06), function(args)
	-- runs between vanilla additive and multiplicative modifiers, so both operations can be in the same hook
	args[1].value = (args[1].value + params.pHmax_add) * params.pHmax_mult
end)

memory.dynamic_hook_mid("RecalculateStats-armor", {"rax"}, {"RValue*"}, 0, ptr:add(0x4187), function(args)
	-- runs between vanilla additive and multiplicative modifiers, so both operations can be in the same hook
	args[1].value = (args[1].value + params.armor_add) * params.armor_mult
end)

memory.dynamic_hook_mid("RecalculateStats-explosive_shot", {"rdi"}, {"RValue*"}, 0, ptr:add(0x433d), function(args)
	args[1].value = args[1].value + params.explosive_shot_add
end)

memory.dynamic_hook_mid("RecalculateStats-lightning", {"rdi"}, {"RValue*"}, 0, ptr:add(0x43be), function(args)
	args[1].value = args[1].value + params.lightning_add
end)

memory.dynamic_hook_mid("RecalculateStats-fire_trail", {"rsi"}, {"RValue*"}, 0, ptr:add(0x4478), function(args)
	args[1].value = args[1].value + params.fire_trail_add
end)

memory.dynamic_hook_mid("RecalculateStats-equipment_cdr", {"rdi"}, {"RValue*"}, 0, ptr:add(0x4675), function(args)
	-- the internal value is unintuitive, so invert it in the API to be a standard multiplier
	args[1].value = 1 - ((1 - args[1].value) * params.equipment_cooldown_mult)
end)

memory.dynamic_hook_mid("RecalculateStats-pVmax", {"rbx"}, {"RValue*"}, 0, ptr:add(0x4811), function(args)
	args[1].value = (args[1].value + params.pVmax_add) * params.pVmax_mult
end)

memory.dynamic_hook_mid("RecalculateStats-pGravity1", {"rbx"}, {"RValue*"}, 0, ptr:add(0x4858), function(args)
	args[1].value = (args[1].value + params.pGravity1_add) * params.pGravity1_mult
end)

memory.dynamic_hook_mid("RecalculateStats-pGravity2", {"rbx"}, {"RValue*"}, 0, ptr:add(0x489f), function(args)
...
-- just a post hook because im sick of mid hooks by this point
-- this kind of sucks but whatever it works i guess
gm.post_script_hook(ActorSkill_recalculate_stats, function(self, other, result, args)
	...
	-- there would most certainly need to be some form of optimisation done for this because this will get called extremely frequently
	gather_params()
	...
	max_stock = max_stock + modifiers.max_stock_add
