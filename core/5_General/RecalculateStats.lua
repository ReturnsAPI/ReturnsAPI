...
Memory.dynamic_hook(_ENV["!guid"], "recalculate_stats", "void*", {"void*", "void*", "void*", "int", "void*"}, gm.get_script_function_address(gm.constants.recalculate_stats),
    -- Pre-hook
...
Memory.dynamic_hook_mid(_ENV["!guid"], "RecalculateStats.knockback_cap", {"rbx"}, {"RValue*"}, 0, ptr:add(0x96d), function(args)
    args[1].value = (args[1].value + params.knockback_cap_add) * params.knockback_cap_mult
end)

Memory.dynamic_hook_mid(_ENV["!guid"], "RecalculateStats.cdr", {"rdi"}, {"RValue*"}, 0, ptr:add(0xa6c), function(args)
...
Memory.dynamic_hook_mid(_ENV["!guid"], "RecalculateStats.maxhp", {"rcx"}, {"RValue*"}, 0, ptr:add(0xd7e), function(args)
...
Memory.dynamic_hook_mid(_ENV["!guid"], "RecalculateStats.hp_regen", {"rax"}, {"RValue*"}, 0, ptr:add(0x22e7), function(args)
    -- no multiplicative regen modifiers in vanilla
    args[1].value = (args[1].value + params.hp_regen_add) * params.hp_regen_mult
end)
Memory.dynamic_hook_mid(_ENV["!guid"], "RecalculateStats.damage", {"rax"}, {"RValue*"}, 0, ptr:add(0x156b), function(args)
    -- runs between vanilla additive and multiplicative modifiers, so both operations can be in the same hook
...
Memory.dynamic_hook_mid(_ENV["!guid"], "RecalculateStats.maxshield", {"rax"}, {"RValue*"}, 0, ptr:add(0x1bdf), function(args)
...
Memory.dynamic_hook_mid(_ENV["!guid"], "RecalculateStats.critical_chance", {"rdx"}, {"RValue*"}, 0, ptr:add(0x28c7), function(args)
...
Memory.dynamic_hook_mid(_ENV["!guid"], "RecalculateStats.attack_speed", {"rdx"}, {"RValue*"}, 0, ptr:add(0x3034), function(args)
...
Memory.dynamic_hook_mid(_ENV["!guid"], "RecalculateStats.pHmax", {"rax"}, {"RValue*"}, 0, ptr:add(0x3c06), function(args)
    -- runs between vanilla additive and multiplicative modifiers, so both operations can be in the same hook
    args[1].value = (args[1].value + params.pHmax_add) * params.pHmax_mult
end)

Memory.dynamic_hook_mid(_ENV["!guid"], "RecalculateStats.armor", {"rax"}, {"RValue*"}, 0, ptr:add(0x4187), function(args)
    -- runs between vanilla additive and multiplicative modifiers, so both operations can be in the same hook
    args[1].value = (args[1].value + params.armor_add) * params.armor_mult
end)

Memory.dynamic_hook_mid(_ENV["!guid"], "RecalculateStats.equipment_cdr", {"rdi"}, {"RValue*"}, 0, ptr:add(0x4675), function(args)
    -- the internal value is unintuitive, so invert it in the API to be a standard multiplier
    args[1].value = 1 - ((1 - args[1].value) * params.equipment_cooldown_mult)
end)

Memory.dynamic_hook_mid(_ENV["!guid"], "RecalculateStats.pVmax", {"rbx"}, {"RValue*"}, 0, ptr:add(0x4811), function(args)
    args[1].value = (args[1].value + params.pVmax_add) * params.pVmax_mult
end)

Memory.dynamic_hook_mid(_ENV["!guid"], "RecalculateStats.pGravity1", {"rbx"}, {"RValue*"}, 0, ptr:add(0x4858), function(args)
    args[1].value = (args[1].value + params.pGravity1_add) * params.pGravity1_mult
end)

Memory.dynamic_hook_mid(_ENV["!guid"], "RecalculateStats.pGravity2", {"rbx"}, {"RValue*"}, 0, ptr:add(0x489f), function(args)
    args[1].value = (args[1].value + params.pGravity2_add) * params.pGravity2_mult
end)

Memory.dynamic_hook_mid(_ENV["!guid"], "RecalculateStats.maxbarrier", {"rax"}, {"RValue*"}, 0, ptr:add(0x5053), function(args)
...
Memory.dynamic_hook(_ENV["!guid"], "ActorSkill.skill_recalculate_stats", "void*", {"void*", "void*", "void*", "int", "void*"}, gm.get_script_function_address(ActorSkill_recalculate_stats),
    -- Pre-hook
    {nil,
